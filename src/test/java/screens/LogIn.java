...
public class LogIn extends ScreenBase{
	...
	public ScreenBase doLogin(String email, String password, String expected1, String expected2) {
		...
		try {
			...
			WebElement myself = wait.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("com.project.WhiteCoat:id/lblMySelf")));
			Assert.assertEquals(expected1, myself.getText());
			
			WebElement mychild = wait.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("com.project.WhiteCoat:id/lblChild")));
