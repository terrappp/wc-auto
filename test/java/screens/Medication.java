...
public class Medication extends ScreenBase{
	...
	public DrugAllergy next() {
		driver.findElement(By.id("com.project.WhiteCoat:id/lblNext")).click();
		return new DrugAllergy(driver);
	}
	...
	public Medication inputMedicationTaken(String medications) {
		if (medications != "") {
			...
			for (String med: medicationList) {
				...
				List<WebElement> meds = driver.findElements(By.id("com.project.WhiteCoat:id/medicineAutoCompeteTextView"));
...
//				driver.pressKey(new KeyEvent(AndroidKey.ENTER));
				...
				List<WebElement> howlongs = driver.findElements(By.id("com.project.WhiteCoat:id/takeMedicineTimeAutoComplete"));
				...
				//dosage: input dosage
				List<WebElement> dosages = driver.findElements(By.id("com.project.WhiteCoat:id/txtDosage"));
				dosages.get(index).sendKeys(detailList[2]);
				
				index++;
				//add more record if not end of list yet
				if (index <= medicationList.length) {
					driver.findElement(By.id("com.project.WhiteCoat:id/addMedicineLayout")).click();
				}
			}	
		}
		return this;
